...
export class UserForm extends React.Component<Props, State> {
  ...
  render() {
    return <Dialog onClose={event => this.close()}>
      <div className="flex flex-column pa4" style={{ width: '400px' }}>
        ...
        {this.state.failed && <div className="error-text ttu mb3 tc">Failed! Try again</div>}
        <TextInput label="Full Name"
          onChange={event => this.onNameChange(event)}
          error={this.state.error.name}
          autoFocus={this.state.user == undefined || this.state.user.name == undefined}
          value={this.state.user && this.state.user.name}></TextInput>
        <TextInput label="Password" type="password"
          onChange={event => this.onPasswordChange(event)}
          error={this.state.error.password}
          autoFocus={this.state.user && this.state.user.name != undefined}></TextInput>
  ...
  validate(): boolean {
    const error = {
      name: this.state.user == undefined || this.state.user.name == undefined || this.state.user.name.trim() === '' ? 'Required' :
        !/^.+\s+.+$/.test(this.state.user.name) ? 'Invalid name' : undefined,
  ...
  submit(event) {
    if (this.validate()) {
      Promise.resolve()
        ...
        .then(() => new SaveUserAction(this.state.user, this.state.password).dispatch())
